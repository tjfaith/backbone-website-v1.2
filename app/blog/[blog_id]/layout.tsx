...
export async function generateMetadata({ params }: { params: { blog_id: string } }): Promise<Metadata> {
    ...
    const singleBlog = await allBlog.find((item: { blog_id: string; }) => item.blog_id === blog_id) || null
    return {
...
export default function SingleBlgLayout({ children }: { children: React.ReactNode }) {
    return <>{children}</>
}
